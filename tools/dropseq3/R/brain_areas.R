...
# - Get ISH -------------------------------------------------------------------
get_ish <- function(gene) {
  gene_id <- cocoframer::get_gene_aba_ish_ids(gene)
  if (length(gene_id) == 0) return(NULL)
  result <- cocoframer::get_aba_ish_structure_data(gene_id[1])
  mutate(result, "gene" = gene)
}

# predict brain area for each cluster based on correlation with ISH
predict_clusters <- function(object, ish_broad) {
  clusters <- sort(unique(object@active.ident))
  get_scores <- function(cluster) {
    mtx <- Matrix::rowMeans(object@assays$RNA@scale.data[
      rownames(ish_broad), object@active.ident == cluster])
    apply(ish_broad, 2, function(x) cor(x, mtx))
  }
  result <- sapply(clusters, get_scores)
